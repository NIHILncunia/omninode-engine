...
- `/docs`는 모든 프로젝트·월드의 설정 문서 중 공개 처리된 문서만 나열하는 전역 목록이다.
- `/`는 `/docs`로 이동하며, `/docs` 목록 열람에는 로그인이 필요 없다.
- 공개 여부의 저장 모델, 실제 목록 API, 문서 상세 공개 범위는 설정 문서 단계에서 문서 CRUD와 함께 확정·구현한다.
- 생성·수정·삭제와 관리자·프로젝트·월드 관리 기능은 인증·권한 검증을 계속 요구한다.
