# Omninode 개발 진행 대장

> 이 파일은 개발 진척도의 단일 기준이다. 새 세션은 먼저 `현재 상태`와 가장 이른 미완료 항목을 확인한다.

## 운영 규칙

- `[x]`는 구현·대상 테스트·필요한 통합 검증까지 끝난 항목에만 사용한다.
- `[ ]`는 아직 시작하지 않았거나, 검증이 남은 항목이다.
- 차단된 항목은 체크하지 않는다. 원인과 재개 조건을 `현재 상태`에 기록한다.
- 기능 단계는 화면만 또는 API만 분리해 완료 처리하지 않는다. 계약, API, UI, 권한, 테스트, 검증을 함께 끝낸다.
- 단계 완료 시 이 파일의 상태·검증 결과·다음 시작 지점을 갱신한다.

## 확정 결정

- [x] 1차 범위: 프로젝트 → 월드 → 카테고리 → 템플릿 → 설정 문서 → 문서 관계로 이어지는 관계형 세계관 관리
- [x] 개발 방식: 기능별 수직 슬라이스(UI·API·권한·검증·테스트 동시 완료)
- [x] 데이터베이스: 개발·운영 PostgreSQL 단일 Drizzle 스키마, 환경별 `DATABASE_URL`
- [x] 인증 진입 경로: `/signin` (`app/pages/signin.vue`), `/login` 미사용
- [x] API 표준 응답: `CreateResponse`의 `error`, `data`, `code`, `message`
- [x] 문서 본문 정본: `documents.content` 하나
- [x] 공개 설정 문서 진입: `/`는 `/docs`로 이동하고, `/docs`는 모든 프로젝트·월드의 공개 문서만 나열하는 비로그인 목록
- [x] 1차 제외: 리비전·비교·롤백·토론·공개 뷰어·실시간 공동 편집·복잡한 감사 로그 화면

## 전체 로드맵

| 상태 | 단계 | 선행 단계 | 핵심 결과 |
| --- | --- | --- | --- |
| 완료 | 0. 기준 정합화·공통 기반 | - | PostgreSQL·응답·`signin` 기준, DB/API/상태 UI 기반 |
| 완료 | 1. 인증·계정 | 0 | 로그인, 토큰 갱신·폐기, 비밀번호 변경 강제 |
| 진행 중 | 2. 관리자·권한 | 1 | 역할·세부 권한·프로젝트 관리자 접근 제어 |
| 진행 전 | 3. 프로젝트 | 2 | 프로젝트 CRUD·소유자 범위 |
| 진행 전 | 4. 월드 | 3 | 월드 CRUD·프로젝트 접근 범위 |
| 진행 전 | 5. 카테고리 | 4 | 3단계 트리·정렬·복구 |
| 진행 전 | 6. 템플릿·섹션 | 5 | 템플릿·섹션 트리·카테고리 매핑 |
| 진행 전 | 7. 설정 문서 | 6 | Markdown 문서·카테고리·섹션 |
| 진행 전 | 8. 관계 타입·문서 관계 | 7 | 역할 기반 관계 생성·검증 |
| 진행 전 | 9. 관계 기반 표현 | 8 | 연표·가계도·문서 관계 조회 |
| 진행 전 | 10. 파생 탐색 | 7, 8 | 검색·최근·즐겨찾기·활동·휴지통 |

## 단계 0 — 기준 정합화·공통 기반

완료 기준: PostgreSQL 단일 기준과 API 표준 계약이 문서·테스트·기반 코드에 일치하고, 이후 수직 슬라이스가 재사용할 최소 공통 기반이 동작한다.

- [x] 통합 설계서의 SQLite 개발·방언 병기·SQLite 검증 표기를 PostgreSQL 단일 기준으로 정정
- [x] 통합 설계서의 성공·목록·오류 응답 예시를 `CreateResponse` 계약으로 정정
- [x] 관리자 라우트 골격 테스트의 `login.vue` 기대값을 `signin.vue`로 정정
- [x] PostgreSQL DB 클라이언트와 환경 변수 예시를 정의
- [x] `/api/health`와 공통 성공·오류 응답 계약을 구현·테스트
- [x] 서버 API 오류 변환의 공통 경계를 확정 (도메인 입력 검증은 각 API 계약 단계에서 확정)
- [x] 인증 상태 Pinia store와 보호 경로 middleware의 최소 인터페이스를 정의
- [x] 공통 로딩·빈 상태·오류 상태 컴포넌트를 정의
- [x] 기본 레이아웃에 실제 `AppSidebar` 연결 범위를 확정·구현
- [x] 대상 테스트, 전체 테스트, 린트, 타입 검사, 빌드 실행 및 결과 기록
- [x] 커밋: `2026 0815 feat: 공통 실행 기반 추가` (`3d69b73`)

## 단계 1 — 인증·계정

완료 기준: 인증되지 않은 접근을 차단하고, 로그인·토큰 갱신·로그아웃·임시 비밀번호 변경 흐름을 UI와 API에서 검증한다.

- [x] 인증 API 계약과 쿠키·토큰 보관 방식 확정
- [x] JWT·비밀번호 해시·refresh token 해시/만료/폐기 서비스 테스트 작성
- [x] `POST /api/auth/signin`, `refresh`, `signout`, `GET me`, `POST password` 구현
- [x] `/signin`, `/account`, `/account/password-change` UI 구현
- [x] 임시 비밀번호 변경 전 일반 보호 경로 차단
- [x] 인증 실패, 비활성·삭제 계정, 권한 없는 접근, 토큰 폐기 테스트
- [x] 대상 테스트, 전체 테스트, 린트, 타입 검사, 빌드 실행 및 결과 기록
- [x] 커밋: `2026 0815 feat: 인증 기반 추가` (`97931cb`)

## 단계 2 — 관리자·권한

완료 기준: 역할 기본 권한과 관리자별 권한이 전역·프로젝트 관리자 관리 UI와 API에 동일하게 적용된다.

### 2.1 권한 계약과 판정 서비스

- [x] 2.1.1 18개 권한 코드와 표시 그룹을 확정한다.
- [ ] 2.1.2 `SUPER_ADMIN`·`ADMIN`·`SUB_ADMIN` 역할별 기본 권한을 확정한다.
- [ ] 2.1.3 관리자별 `adminPermissions` override 규칙을 확정한다.
- [ ] 2.1.4 프로젝트 소유자·활성 `projectAdmins` 배정 범위를 확정한다.
- [ ] 2.1.5 권한 코드·역할 기본값 타입과 정적 데이터를 구현한다.
- [ ] 2.1.6 권한 판정 서비스 실패 테스트를 작성한다.
- [ ] 2.1.7 권한 판정 서비스를 구현한다.
- [ ] 2.1.8 override·프로젝트 범위·역할 범위 밖 권한 거부 테스트를 통과시킨다.

### 2.2 관리자·권한 서비스

- [ ] 2.2.1 관리자 목록·상세·이메일 중복·복구 Repository 계약을 작성한다.
- [ ] 2.2.2 관리자 생성·수정·소프트 삭제 Repository를 구현한다.
- [ ] 2.2.3 권한 마스터·관리자별 권한 override Repository를 구현한다.
- [ ] 2.2.4 프로젝트 관리자 배정·복구·비활성화 Repository를 구현한다.
- [ ] 2.2.5 관리자 생성·수정·삭제 서비스 테스트를 작성한다.
- [ ] 2.2.6 임시 비밀번호·비밀번호 변경 필요 상태를 구현한다.
- [ ] 2.2.7 마지막 `SUPER_ADMIN` 삭제와 자기 비활성화를 차단한다.
- [ ] 2.2.8 권한 조회·수정 서비스와 역할 범위 밖 권한 거부를 구현한다.
- [ ] 2.2.9 관리자·권한 서비스 테스트를 통과시킨다.

### 2.3 관리자·권한 API

- [ ] 2.3.1 관리자 목록·생성 API 계약 테스트를 작성한다.
- [ ] 2.3.2 관리자 상세·수정·삭제 API 계약 테스트를 작성한다.
- [ ] 2.3.3 권한 마스터·관리자 권한 조회·수정 API 계약 테스트를 작성한다.
- [ ] 2.3.4 입력 검증·인증·권한 오류 응답을 구현한다.
- [ ] 2.3.5 관리자·권한 API를 구현한다.
- [ ] 2.3.6 비밀번호 해시와 임시 비밀번호가 응답에 포함되지 않는지 검증한다.
- [ ] 2.3.7 API 테스트를 통과시킨다.

### 2.4 전역 관리자 UI

- [ ] 2.4.1 관리자 목록 컴포넌트와 검색·페이지 상태를 구현한다.
- [ ] 2.4.2 관리자 생성 폼을 구현한다.
- [ ] 2.4.3 관리자 상세·수정 화면을 구현한다.
- [ ] 2.4.4 관리자 권한 조회·수정 화면을 구현한다.
- [ ] 2.4.5 로딩·빈 상태·오류 상태를 연결한다.
- [ ] 2.4.6 `SUB_ADMIN`의 관리자 화면·메뉴 비노출을 검증한다.
- [ ] 2.4.7 관리자 UI 테스트와 타입 검사를 통과시킨다.

### 2.5 프로젝트 관리자 API·UI

- [ ] 2.5.1 프로젝트 관리자 초대·재초대 계약 테스트를 작성한다.
- [ ] 2.5.2 프로젝트 관리자 비활성화·배정 해제 계약 테스트를 작성한다.
- [ ] 2.5.3 `ADMIN`의 소유 프로젝트 범위와 `SUPER_ADMIN` 전체 범위를 검증한다.
- [ ] 2.5.4 `SUB_ADMIN` 및 타 프로젝트 관리자의 접근을 차단한다.
- [ ] 2.5.5 삭제된 동일 배정의 복구 규칙을 구현한다.
- [ ] 2.5.6 프로젝트 관리자 API를 구현한다.
- [ ] 2.5.7 프로젝트 관리자 목록·초대 UI를 구현한다.
- [ ] 2.5.8 재초대·비활성화·배정 해제 UI를 구현한다.
- [ ] 2.5.9 프로젝트 관리자 API·UI 테스트를 통과시킨다.

### 2.6 통합 검증과 완료 기록

- [ ] 2.6.1 목록에서 권한 없는 관리자가 노출되지 않는지 검증한다.
- [ ] 2.6.2 검색 결과에서 권한 없는 관리자가 노출되지 않는지 검증한다.
- [ ] 2.6.3 직접 URL 접근 시 권한 없는 대상의 존재를 숨기는지 검증한다.
- [ ] 2.6.4 API 응답이 권한 없는 대상의 존재를 숨기는지 검증한다.
- [ ] 2.6.5 대상 테스트를 실행하고 결과를 기록한다.
- [ ] 2.6.6 전체 테스트를 실행하고 결과를 기록한다.
- [ ] 2.6.7 린트·타입 검사·빌드를 실행하고 결과를 기록한다.
- [ ] 2.6.8 `TODO.md`와 관련 Superpowers 계획의 체크 상태를 갱신한다.
- [ ] 2.6.9 커밋: `2026 0816 feat: 관리자 권한 관리 추가`


## 단계 3 — 프로젝트

- [ ] 프로젝트 CRUD 계약·소유자 범위·소프트 삭제 복구 규칙 확정
- [ ] 프로젝트 서비스·API 테스트 작성 및 구현
- [ ] 프로젝트 목록·생성·상세·설정 UI 구현
- [ ] 실제 API 없는 더미 통계를 사용하지 않는 대시보드 구현
- [ ] 공통 검증 실행·결과 기록
- [ ] 커밋: `yyyy MMdd feat: 프로젝트 관리 추가`

## 단계 4 — 월드

- [ ] 프로젝트 하위 월드 CRUD·접근 범위·상위 삭제 접근 차단 계약 확정
- [ ] 월드 서비스·API·권한 테스트 작성 및 구현
- [ ] 월드 목록·생성·대시보드·설정 UI 구현
- [ ] 공통 검증 실행·결과 기록
- [ ] 커밋: `yyyy MMdd feat: 월드 관리 추가`

## 단계 5 — 카테고리

- [ ] 같은 월드 상위 항목, 1~3단계, 최상위 템플릿 연결 규칙 확정
- [ ] 생성·수정·삭제·정렬·동일 이름 복구 서비스 테스트 및 구현
- [ ] 카테고리 목록·생성·상세와 문서·템플릿 연결 UI 구현
- [ ] 상위 삭제 시 하위 접근 차단 검증
- [ ] 공통 검증 실행·결과 기록
- [ ] 커밋: `yyyy MMdd feat: 카테고리 관리 추가`

## 단계 6 — 템플릿·섹션

- [ ] 템플릿 버전·섹션 트리·필수 여부·카테고리 매핑 계약 확정
- [ ] 템플릿·섹션 서비스와 API 테스트 작성 및 구현
- [ ] 템플릿 목록·생성·상세·편집·카테고리·문서 UI 구현
- [ ] 구버전 경고와 삭제 템플릿 섹션 표시 규칙 검증
- [ ] 공통 검증 실행·결과 기록
- [ ] 커밋: `yyyy MMdd feat: 템플릿 관리 추가`

## 단계 7 — 설정 문서

- [ ] 문서 CRUD, 정확히 하나의 1단계 카테고리, 템플릿 버전·섹션 계약 확정
- [ ] Markdown 분해·재결합과 단일 본문 정본 서비스 테스트 작성 및 구현
- [ ] 문서 목록·생성·상세·편집 UI 구현
- [ ] 자동 저장 없이 명시적 저장만 수행하도록 검증
- [ ] 공통 검증 실행·결과 기록
- [ ] 커밋: `yyyy MMdd feat: 설정 문서 관리 추가`

## 단계 8 — 관계 타입·문서 관계

- [ ] 관계 타입, 역할, 허용 카테고리, 대상 수·필수 역할 계약 확정
- [ ] 같은 월드·역할 호환성·대상 수 검증 트랜잭션 테스트 및 구현
- [ ] 관계 타입·관계 목록·생성·상세·수정 UI 구현
- [ ] 문서 중심 연결 문서 탐색 UI 구현
- [ ] 공통 검증 실행·결과 기록
- [ ] 커밋: `yyyy MMdd feat: 문서 관계 관리 추가`

## 단계 9 — 관계 기반 표현

- [ ] 연표·가계도·관계 조회의 입력 데이터와 도출 규칙 확정
- [ ] 별도 원본 데이터를 저장하지 않는 조회 서비스 테스트 및 구현
- [ ] 프로젝트·월드·문서 범위 관계 표현 UI 구현
- [ ] 범용 전체 그래프를 기본 기능으로 만들지 않는 범위 검증
- [ ] 공통 검증 실행·결과 기록
- [ ] 커밋: `yyyy MMdd feat: 관계 기반 표현 추가`

## 단계 10 — 파생 탐색

- [ ] 검색·최근·즐겨찾기·활동·휴지통의 데이터 모델과 개인정보·권한 범위 설계 승인
- [ ] 검색 대상과 정렬·필터·페이지 계약 구현
- [ ] 파생 조회 API·UI·권한·테스트 구현
- [ ] 공통 검증 실행·결과 기록
- [ ] 커밋: `yyyy MMdd feat: 파생 탐색 기능 추가`

## 현재 상태

- 마지막 완료: 단계 1 인증·계정 구현·검증 완료
- 현재 단계: 2. 관리자·권한
- 진행 위치: 2.1 권한 계약과 판정 서비스
- 다음 시작 항목: 2.1.2 `SUPER_ADMIN`·`ADMIN`·`SUB_ADMIN` 역할별 기본 권한 확정
- 검증 결과: 2026-08-15 `pnpm test` 18개 파일·74개 테스트 통과, `pnpm exec vue-tsc --noEmit` 및 `pnpm build` 통과. 단계 1 변경 파일 대상 린트는 통과.
- 차단 사항: 전체 린트는 기존 무관 오류 24건(`Home.vue`, query composable·테스트의 형식 오류)으로 실패.

## 참고 문서

- `references/옴니노드 프로젝트 기본 지침.md`
- `references/옴니노드 사이트맵 주소 후보.md`
- `references/옴니노드_UI_API_통합_작업_설계서.md`
- `docs/superpowers/specs/2026-08-15-development-roadmap-design.md`
- `docs/superpowers/specs/2026-08-16-admin-permission-management-design.md`
- `docs/superpowers/plans/2026-08-16-admin-permission-management.md`
