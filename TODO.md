...
## 단계 2 — 어드민 요청·관리자·권한 정정 (후속 정정 진행 중)
...
- [ ] 활성 배정·권한 행 동시 소프트 삭제·복구와 프로젝트별 권한 판정을 정정·테스트
...
- [ ] 삭제 ADMIN 재승인 transaction과 최신 Vue Query/store 상태를 구현·검증
- [ ] 권한 마스터 seed·역할 기본 권한 제거·TODO 및 완료 리포트를 동기화
- [ ] 후속 정정 검증·완료 리포트 갱신·커밋
...
- 마지막 완료: 단계 2 초기 구현·검증 완료, 후속 권한 정정 진행 중
- 현재 단계: 2. 어드민 요청·관리자·권한 정정
- 진행 위치: 활성 배정·권한 행 수명, 삭제 ADMIN 재승인, 최신 상태 이관 정정
- 다음 시작 항목: 정정 설계·계획 검토 후 테스트 우선 구현
- 검증 결과: 후속 정정 전 기준으로 83개 테스트, lint, 타입 검사, 빌드를 통과했다.
- 차단 사항: 없음. 후속 정정 완료 뒤 단계 3 프로젝트 CRUD를 시작한다.
