...
| 상태 | 단계 | 선행 단계 | 핵심 결과 |
| --- | --- | --- | --- |
| 진행 전 | 0. 기준 정합화·공통 기반 | - | PostgreSQL·응답·`signin` 기준, DB/API/상태 UI 기반 |
| 진행 전 | 1. 인증·계정 | 0 | 로그인, 토큰 갱신·폐기, 비밀번호 변경 강제 |
...
- [ ] 서버 입력 검증과 API 오류 변환의 공통 경계를 확정
- [ ] 인증 상태 Pinia store와 보호 경로 middleware의 최소 인터페이스를 정의
- [ ] 공통 로딩·빈 상태·오류 상태 컴포넌트를 정의
- [ ] 기본 레이아웃에 실제 `AppSidebar` 연결 범위를 확정·구현
- [ ] 대상 테스트, 전체 테스트, 린트, 타입 검사, 빌드 실행 및 결과 기록
- [ ] 커밋: `yyyy MMdd feat: 공통 실행 기반 추가`

## 단계 1 — 인증·계정

완료 기준: 인증되지 않은 접근을 차단하고, 로그인·토큰 갱신·로그아웃·임시 비밀번호 변경 흐름을 UI와 API에서 검증한다.

- [ ] 인증 API 계약과 쿠키·토큰 보관 방식 확정
- [ ] JWT·비밀번호 해시·refresh token 해시/만료/폐기 서비스 테스트 작성
- [ ] `POST /api/auth/signin`, `refresh`, `signout`, `GET me`, `POST password` 구현
- [ ] `/signin`, `/account`, `/account/password-change` UI 구현
- [ ] 임시 비밀번호 변경 전 일반 보호 경로 차단
- [ ] 인증 실패, 비활성·삭제 계정, 권한 없는 접근, 토큰 폐기 테스트
- [ ] 대상 테스트, 전체 테스트, 린트, 타입 검사, 빌드 실행 및 결과 기록
- [ ] 커밋: `yyyy MMdd feat: 인증 기반 추가`
...
- 마지막 완료: PostgreSQL DB 팩터리와 `/api/health` 표준 응답 구현·대상 검증 (미커밋)
- 현재 단계: 0. 기준 정합화·공통 기반
- 다음 시작 항목: 인증 상태 Pinia store와 보호 경로 middleware의 최소 인터페이스 구현
- 차단 사항: 없음
